...
df_orig.to_csv('stageTiming.csv', columns=['pre-job input', 'opening input file', 'upto appmgr start', 'initialisation', 'event loop', 'ending'])
...
for subdir, dirs, files in os.walk('/work/d60/d60/shared/optimisation/benchmark/test1/output'):
    for file in files:
        ...
        if filepath.endswith('.EVNTtoHITS'):
            ...
            df = pd.DataFrame()
            df = pd.read_csv('stageTiming.csv')
